...
load scripts
```{r}
source("C:/Users/LinusWidmer/Documents/word_catalog.R")
```
...
## read data
```{r}
df = read.csv("C:/Users/LinusWidmer/Downloads/results-survey371132.csv")
```
...
```{r}
compute_aggressive_score <- function(row) {
  count <- 0
  total <- length(WFCT_aggression_items)
...
df$aggressiveWordCompletionScore <- apply(df, 1, compute_aggressive_score)
```
