...
```{r}
plotBox("aggressiveWordCompletionScore", "threatCondition")
```

```{r}
summary_mean_sd(df, "aggressiveWordCompletionScore", "threatCondition")
```

## 7.2 Prerequisites for Inference Statistics

### 7.2.1 Assumption 1: Normality withing each group 

Check Visually
```{r}
agg_qq <- check_normality_qq(
  df, aggressiveWordCompletionScoreTransformed,
  threatCondition
)
agg_hist <- check_normality_hist(
  df, aggressiveWordCompletionScoreTransformed,
  threatCondition
)

plotHist("aggressiveWordCompletionScore", c(0, 1), condition = "threatCondition", binwidth = 0.1)

grid.arrange(agg_qq, agg_hist, ncol = 1)

check_normality_qq(
  df, aggressiveWordCompletionScore,
  threatCondition
)
```
qq-plot -> data points should be roughly on the line
hist -> should roughly resemble a normal distribution

### 7.2.2 Assumption 2: Homogeneity of variance
...
- Normality within each group is not given. 
A visual inspection shows a zero inflated distribution
...
df$SuccessCount <- df$aggressiveWordCompletionScore * TotalTrials 

df$FailureCount <- TotalTrials - df$SuccessCount

df$SuccessFailureMatrix <- cbind(df$SuccessCount, df$FailureCount)

glm_model <- glm(
  SuccessFailureMatrix ~ threatCondition, 
  data = df, 
...
cat("Calculated Cook's Distance Threshold:", round(threshold, 4), "\n")

influential_indices <- which(cooks_dist > threshold)

cat("\n--- Extreme Outliers (Exceeding Threshold) ---\n")
if (length(influential_indices) > 0) {
  print(df[influential_indices, ]) 
  
  cat("\nCook's Distance values for these points:\n")
  print(cooks_dist[influential_indices])
} else {
  cat("No observations exceeded the Cook's Distance threshold of", round(threshold, 4), "\n")
}
```


```{r}
is_not_influential <- cooks_dist <= threshold

df_clean <- df[is_not_influential, ]

cat("\n--- Data Cleaning Summary ---\n")
cat("Original rows:", N, "\n")
cat("Excluded rows (outliers):", length(influential_indices), "\n")
cat("Cleaned rows:", nrow(df_clean), "\n")
```

```{r}
summary_mean_sd(df_clean, "aggressiveWordCompletionScore", "threatCondition")
```

```{r}
# Rerun the GLM on the cleaned data
glm_model_clean <- glm(
  SuccessFailureMatrix ~ threatCondition, 
  data = df_clean, 
  family = quasibinomial
