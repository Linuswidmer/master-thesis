...
- Since assumptions are violated: Quasibinomial GLM on proportion data
...
```{r}
agg_qq <- check_normality_qq(df, aggressiveWordCompletionScore, threatCondition)
agg_hist <- check_normality_hist(df, aggressiveWordCompletionScore, threatCondition)
grid.arrange(agg_qq, agg_hist, ncol = 1)
```

**Interpretation:**

- The distribution shows zero-inflation (many participants with 0 aggressive completions)
- Normality assumption is violated, therefore a t-test is not appropriate

### 7.2.2 Homogeneity of Variance

```{r}
check_homogeneity(df, aggressiveWordCompletionScore, threatCondition)
```

## 7.3 Inference Statistics: Quasibinomial GLM

Since the outcome is a proportion (number of aggressive completions out of 10 trials)
and assumptions for parametric tests are violated, we use a quasibinomial GLM.
The quasibinomial family accounts for overdispersion in binomial data.
...
### 7.3.2 Model Diagnostics: Cook's Distance
...
### 7.3.3 Sensitivity Analysis: Model without Influential Observations
...
```{r}
glm_summary_table(glm_agg$model, "Aggressive Cognition GLM (Full Sample)") %>%
...
```{r}
anx_qq <- check_normality_qq(df, anxiousWordCompletionScore, threatCondition)
anx_hist <- check_normality_hist(df, anxiousWordCompletionScore, threatCondition)
grid.arrange(anx_qq, anx_hist, ncol = 1)
```

**Interpretation:**

- Similar to aggressive cognition, the distribution shows zero-inflation
- Normality assumption is violated, therefore a t-test is not appropriate

### 8.2.2 Homogeneity of Variance

```{r}
check_homogeneity(df, anxiousWordCompletionScore, threatCondition)
```

## 8.3 Inference Statistics: Quasibinomial GLM
...
### 8.3.2 Model Diagnostics: Cook's Distance
...
### 8.3.3 Sensitivity Analysis: Model without Influential Observations
...
```{r}
glm_summary_table(glm_anx$model, "Anxious Cognition GLM (Full Sample)") %>%
...
- Since assumptions are violated: Quasibinomial GLM with interaction term
...
- As established in Section 7, the outcome shows zero-inflation
- Linear regression assumptions are violated, therefore we use a quasibinomial GLM

## 9.3 Inference Statistics: Quasibinomial GLM with Moderation
...
```{r}
glm_agg_mod <- run_quasibinomial_glm_moderation(
  data = df,
  score_col = "aggressiveWordCompletionScore",
  predictor = "threatCondition",
...
```{r}
glm_summary_table(glm_agg_mod$model, "Aggressive Cognition Moderation GLM") %>%
...
```{r}
anx_mod_hist <- check_normality_hist(df, anxiousWordCompletionScore, threatCondition)
anx_mod_hist
```

**Interpretation:**

- As established in Section 8, the outcome shows zero-inflation
- Linear regression assumptions are violated, therefore we use a quasibinomial GLM

## 10.3 Inference Statistics: Quasibinomial GLM with Moderation
...
```{r}
glm_anx_mod <- run_quasibinomial_glm_moderation(
  data = df,
  score_col = "anxiousWordCompletionScore",
  predictor = "threatCondition",
...
```{r}
glm_summary_table(glm_anx_mod$model, "Anxious Cognition Moderation GLM") %>%
