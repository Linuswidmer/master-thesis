...
loadings_matrix <- as.data.frame(efa_result_twof$loadings[seq_along(mmb_labels), ])
...
loading_table %>%
  ...
  row_spec(seq_len(nrow(loading_table)), background = "white", color = "black")
```
...
### 4.1.6 Two-Factor Correlated + remove MMBi9 + Residuals CFA

- 2 latent factors
- correlation allowed between factors
- remove item MMBi9, because it poorly fits into the concept.
- allow correlated residuals item 7 and 8
```{r }
model_twof_cor_mod <- "
  # latent variables
  Pressured =~ MMBi1 + MMBi2 + MMBi3 + MMBi4 + MMBi5
  Autonomous =~ MMBi6 + MMBi7 + MMBi8
...
# Compute McDonald's Omega for each factor using compRelSEM
omega <- compRelSEM(fit_twof_cor_mod, tau.eq = FALSE, ord.scale = FALSE, return.df = TRUE)

cat("McDonald's Omega Reliability Coefficients:\n")
print(omega)
```
...
df$rowmeansAutonomous <- rowMeans(
  df[setdiff(mmb_autonomous_items, "MMBi9")], # exclude item 9
  na.rm = TRUE
)
```
...
```{r}
df$pressuredMotivation <- df$rowmeansPressured
df$autonomousMotivation <- df$rowmeansAutonomous
```
...
```{r}
summary_binary_counts(df, WFCT_aggression_items)
```

**Interpretation**

- *Low Positive Response Rates:* Several items show low positive (aggressive) completion rates
- *Missing Values:* Some missings on items 7, 8, 9, 10
...
df$aggressiveCognition <- df$aggressiveWordCompletionScore * 100
```
...
```{r}
plotHist("aggressiveCognition", c(0, 100), condition = NULL, binwidth = 1)
```
...
```{r}
summary_binary_counts(df, WFCT_anxiety_items)
```

**Interpretation**

- *Low Positive Response Rates:* Several items show low positive (anxious) completion rates
- *Missing Values:* ...
...
df$anxiousCognition <- df$anxiousWordCompletionScore * 100
```

### 6.3.2 Visualization & Descriptives
```{r}
summary_mean_sd(df, c("anxiousCognition"))
```

```{r}
plotHist("anxiousCognition", c(0, 100), condition = NULL, binwidth = 1)
```
...
### 7.3.6 Effect Size for Post-Hoc Power Analysis

Cohen's d for comparing threat vs. no-threat groups:
```{r}
m1_agg <- mean(df$aggressiveCognition[df$threatCondition == "threat"], na.rm = TRUE)
m2_agg <- mean(df$aggressiveCognition[df$threatCondition == "noThreat"], na.rm = TRUE)
sd1_agg <- sd(df$aggressiveCognition[df$threatCondition == "threat"], na.rm = TRUE)
sd2_agg <- sd(df$aggressiveCognition[df$threatCondition == "noThreat"], na.rm = TRUE)
n1_agg <- sum(df$threatCondition == "threat" & !is.na(df$aggressiveCognition))
n2_agg <- sum(df$threatCondition == "noThreat" & !is.na(df$aggressiveCognition))

sd_pooled_agg <- sqrt(((n1_agg - 1) * sd1_agg^2 + (n2_agg - 1) * sd2_agg^2) / (n1_agg + n2_agg - 2))
cohens_d_h1a <- (m1_agg - m2_agg) / sd_pooled_agg

cat("=== H1a: Effect Size for G*Power ===\n")
cat("M (threat):", round(m1_agg, 3), "\n")
cat("M (noThreat):", round(m2_agg, 3), "\n")
cat("SD pooled:", round(sd_pooled_agg, 3), "\n")
cat("Cohen's d:", round(cohens_d_h1a, 3), "\n")
cat("n (threat):", n1_agg, "\n")
cat("n (noThreat):", n2_agg, "\n")
```
...
```{r}
plotBox("anxiousCognition", "threatCondition")
```

```{r}
summary_mean_sd(df, "anxiousCognition", "threatCondition")
```
...
### 8.3.6 Effect Size for Post-Hoc Power Analysis

Cohen's d for comparing threat vs. no-threat groups:
```{r}
m1_anx <- mean(df$anxiousCognition[df$threatCondition == "threat"], na.rm = TRUE)
m2_anx <- mean(df$anxiousCognition[df$threatCondition == "noThreat"], na.rm = TRUE)
sd1_anx <- sd(df$anxiousCognition[df$threatCondition == "threat"], na.rm = TRUE)
sd2_anx <- sd(df$anxiousCognition[df$threatCondition == "noThreat"], na.rm = TRUE)
n1_anx <- sum(df$threatCondition == "threat" & !is.na(df$anxiousCognition))
n2_anx <- sum(df$threatCondition == "noThreat" & !is.na(df$anxiousCognition))

sd_pooled_anx <- sqrt(((n1_anx - 1) * sd1_anx^2 + (n2_anx - 1) * sd2_anx^2) / (n1_anx + n2_anx - 2))
cohens_d_h1b <- (m1_anx - m2_anx) / sd_pooled_anx

cat("=== H1b: Effect Size for G*Power ===\n")
cat("M (threat):", round(m1_anx, 3), "\n")
cat("M (noThreat):", round(m2_anx, 3), "\n")
cat("SD pooled:", round(sd_pooled_anx, 3), "\n")
cat("Cohen's d:", round(cohens_d_h1b, 3), "\n")
cat("n (threat):", n1_anx, "\n")
cat("n (noThreat):", n2_anx, "\n")
```
...
### 9.3.6 Effect Size for Post-Hoc Power Analysis

Cohen's f² for the interaction term (R² change when adding interaction):
```{r}
# Model without interaction
model_h2a_reduced <- lm(aggressiveCognition ~ threatCondition + pressuredMotivation, data = df)
# Model with interaction
model_h2a_full <- lm(aggressiveCognition ~ threatCondition * pressuredMotivation, data = df)

r2_h2a_reduced <- summary(model_h2a_reduced)$r.squared
r2_h2a_full <- summary(model_h2a_full)$r.squared
r2_h2a_change <- r2_h2a_full - r2_h2a_reduced

f2_h2a <- r2_h2a_change / (1 - r2_h2a_full)
n_h2a <- nrow(model.frame(model_h2a_full))

cat("=== H2a: Effect Size for G*Power ===\n")
cat("R² (reduced model, no interaction):", round(r2_h2a_reduced, 4), "\n")
cat("R² (full model, with interaction):", round(r2_h2a_full, 4), "\n")
cat("R² change:", round(r2_h2a_change, 4), "\n")
cat("Cohen's f²:", round(f2_h2a, 4), "\n")
cat("Total N:", n_h2a, "\n")
cat("Number of tested predictors: 1 (interaction)\n")
cat("Total number of predictors: 3 (threat + pressured + interaction)\n")
```
...
### 10.3.6 Effect Size for Post-Hoc Power Analysis

Cohen's f² for the interaction term (R² change when adding interaction):
```{r}
# Model without interaction
model_h2b_reduced <- lm(anxiousCognition ~ threatCondition + autonomousMotivation, data = df)
# Model with interaction
model_h2b_full <- lm(anxiousCognition ~ threatCondition * autonomousMotivation, data = df)

r2_h2b_reduced <- summary(model_h2b_reduced)$r.squared
r2_h2b_full <- summary(model_h2b_full)$r.squared
r2_h2b_change <- r2_h2b_full - r2_h2b_reduced

f2_h2b <- r2_h2b_change / (1 - r2_h2b_full)
n_h2b <- nrow(model.frame(model_h2b_full))

cat("=== H2b: Effect Size for G*Power ===\n")
cat("R² (reduced model, no interaction):", round(r2_h2b_reduced, 4), "\n")
cat("R² (full model, with interaction):", round(r2_h2b_full, 4), "\n")
cat("R² change:", round(r2_h2b_change, 4), "\n")
cat("Cohen's f²:", round(f2_h2b, 4), "\n")
cat("Total N:", n_h2b, "\n")
cat("Number of tested predictors: 1 (interaction)\n")
cat("Total number of predictors: 3 (threat + autonomous + interaction)\n")
```

# 11. Post Hoc Exploratory Analysis: MMB Effects on Self-Evaluated Gender Knowledge

*Rationale:*

Given the poor psychometric properties of the WFCT scales for aggressive and anxious cognition
(KMO values of .16 and .30, respectively), we explore whether Motivation for Masculine Behaviour
(MMB) predicts self-evaluated gender knowledge (GKSELF) as an alternative outcome measure.
GKSELF demonstrated good properties in the manipulation check and provides a more reliable
measure of participants' response to the experimental context.

*Objectives:*

- Assess if pressured motivation for masculine behaviour predicts self-evaluated gender knowledge
- Assess if autonomous motivation for masculine behaviour predicts self-evaluated gender knowledge

*Methodology:*

- Visualization & Descriptives (same style as hypotheses 2a & 2b)
- Linear regression with GKSELF as continuous outcome
- Two-tailed tests (exploratory, no directional assumptions)

## 11.1 Effect of Pressured Motivation on GKSELF
...
```{r}
summary_mean_sd(df, "GKSELF", "threatCondition")
```

### 11.1.2 Inference Statistics: Linear Regression

```{r}
lm_gkself_pressured <- lm(GKSELF ~ pressuredMotivation, data = df)
summary(lm_gkself_pressured)
```

### 11.1.3 Model Diagnostics

```{r}
par(mfrow = c(2, 2))
plot(lm_gkself_pressured)
par(mfrow = c(1, 1))
```
...
## 11.2 Effect of Autonomous Motivation on GKSELF

### 11.2.1 Visualization

```{r}
plotLine("autonomousMotivation", "GKSELF",
  condition = "threatCondition", df = df
)
```

### 11.2.2 Inference Statistics: Linear Regression

```{r}
lm_gkself_autonomous <- lm(GKSELF ~ autonomousMotivation, data = df)
summary(lm_gkself_autonomous)
```
...
```{r}
save_corrplot_apa(cor_mmb, "corrplot-mmb", cluster_groups = c(1, 5, 6, 9))

# Rename items for cleaner labels (remove "WFCT" prefix)
cor_aggr_renamed <- cor_aggr
rownames(cor_aggr_renamed) <- gsub("^WFCT", "", rownames(cor_aggr_renamed))
colnames(cor_aggr_renamed) <- gsub("^WFCT", "", colnames(cor_aggr_renamed))
save_corrplot_apa(cor_aggr_renamed, "corrplot-aggressive-cognition")

cor_anx_renamed <- cor_anx
rownames(cor_anx_renamed) <- gsub("^WFCT", "", rownames(cor_anx_renamed))
colnames(cor_anx_renamed) <- gsub("^WFCT", "", colnames(cor_anx_renamed))
save_corrplot_apa(cor_anx_renamed, "corrplot-anxious-cognition")
```

### Masculinity Threat Effects

Aggressive word completions by threat condition
```{r}
agg_by_threat_boxplot <- plotBox(
  "aggressiveCognition",
  "threatCondition",
  x_label = "",
  y_label = "Aggressive Word Completion Score (%)",
  var_labels = c("aggressiveCognition" = ""),
  fill_labels = c(
    "noThreat" = "Control",
    "threat" = "Threat"
  ),
...
agg_by_threat_boxplot
save_plot(agg_by_threat_boxplot, "aggressive-cognition-by-threat-boxplot")
```

Anxious word completions by threat condition
```{r}
anx_by_threat_boxplot <- plotBox(
  "anxiousCognition",
  "threatCondition",
  x_label = "",
  y_label = "Anxious Word Completion Score (%)",
  var_labels = c("anxiousCognition" = ""),
  fill_labels = c(
    "noThreat" = "Control",
    "threat" = "Threat"
  ),
...
anx_by_threat_boxplot
save_plot(anx_by_threat_boxplot, "anxious-cognition-by-threat-boxplot")
```

Combined plot (for reference)
```{r}
agg_anx_by_threat_boxplot <- plotBox(
  c("aggressiveCognition", "anxiousCognition"),
  "threatCondition",
  x_label = "Cognition Type",
  y_label = "Word Completion Score (%)",
  var_labels = c(
    "aggressiveCognition" = "Aggressive",
    "anxiousCognition" = "Anxious"
  ),
  fill_labels = c(
    "noThreat" = "Control",
    "threat" = "Threat"
...
### Modertation of Masculinity Threat Effects

```{r}
agg_moderation_pressured_motivation <- plotModeration(
  x = "pressuredMotivation",
  y = "aggressiveCognition",
  condition = "threatCondition",
  df = df,
  x_label = "Pressured Motivation Score",
  y_label = "Agg. Word Completion Score (%)",
  condition_labels = c(
    "noThreat" = "Control",
    "threat" = "Threat"
  ),
...
agg_moderation_pressured_motivation
save_plot(agg_moderation_pressured_motivation, "aggressive-cognition-pressured-motivation-line-chart")
```

```{r}
anx_moderation_autonomous_motivation <- plotModeration(
  x = "autonomousMotivation",
  y = "anxiousCognition",
  condition = "threatCondition",
  df = df,
  x_label = "Autonomous Motivation Score",
  y_label = "Anx. Word Completion Score (%)",
  condition_labels = c(
    "noThreat" = "Control",
    "threat" = "Threat"
  ),
...
anx_moderation_autonomous_motivation
save_plot(anx_moderation_autonomous_motivation, "anxious-cognition-autonomous-motivation-line-chart")
```
...
```{r}
# Example Usage:
histogram_age <- plotHistAPA(
  score_col = "AGE",
  df = df,
  x_range = c(18, 70),
  binwidth = 2,
  x_label = "Participant Age (Years)",
  y_label = "Frequency",
  legend_title = "Group",
  legend_position = "top.right"
)
save_plot(histogram_age, "sample-histogram-age")
```


```{r}
histogram_gkfeedback <- plotHistAPA(
  score_col = "GKFEEDBACK",
  df = df,
  x_range = c(1, 10),
  binwidth = 1,
  x_label = "Score",
  y_label = "Frequency",
  condition = "threatCondition",
  condition_labels = c("noThreat" = "Control", "threat" = "Threat"),
  legend_title = "Group",
  legend_position = "top.right"
)
histogram_gkfeedback
save_plot(histogram_gkfeedback, "histogram-gkfeedback")

histogram_gkself <- plotHistAPA(
  score_col = "GKSELF",
  df = df,
  x_range = c(1, 10),
  binwidth = 1,
...
histogram_gkself
save_plot(histogram_gkself, "histogram-gkself")
```