...
compute rowmeans either way for descriptive analysis 
...
Compute scores
<!-- ```{r}
df$regAutonomous <- rowMeans(
  df[mmb_autonomous_items],
  na.rm = TRUE
)

df$regPressured <- rowMeans(
  df[mmb_pressured_items],
  na.rm = TRUE
)
``` -->
...
### Row Means
```{r}
summary_mean_sd(df, c("rowmeansAutonomous", "rowmeansPressured"))
```

```{r}
plotHist("rowmeansAutonomous", c(0, 7))
plotHist("rowmeansPressured", c(0, 7))

p <- plotBox(c("rowmeansAutonomous", "rowmeansPressured"))
ggpar(p, ylim = c(0,7))
```
...
```{r}
summary_mean_sd(df, c("regAutonomous", "regPressured"))
```

```{r}
plotHist("regAutonomous", c(-5, 5))
plotHist("regPressured", c(-5, 5))

p <- plotBox(c("regAutonomous", "regPressured"))
ggpar(p, ylim = c(-5, 5))
```
