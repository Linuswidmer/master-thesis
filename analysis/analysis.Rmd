---
title: 'Analysis script for master thesis: Motivated Responses to a Masculinity Threat
...
### 2.2.1 Gender Knowledge Feedback
...
### 2.2.3 Self evaluation of Gender Knowledge
...
### 2.2.5 Motivation for Masculine Behaviour
...
### 2.2.6 Aggressive Cognition

```{r warning = FALSE, fig.width=10, fig.height=12, fig.align='center'}
...
# 3. Test Quality Criteria: Motivation for Masculine Behaviour
*Objectives:*
...
### 3.1.1 Visualization & Descriptives
...
### 3.1.2 Check sampling adequacy for Factory Analysis
...
### 3.1.3 Two-Factor Uncorrelated CFA
...
### 3.1.5 Two-Factor Correlated CFA
...
#### 3.1.6 Two-Factor Correlated + Residuals CFA
...
## 3.3 Latent Trait Scores

### 3.3.1 Compute Scores
...
### 3.3.2 Visualization & Descriptives
```{r}
...
# 4. Test Quality Criteria: Aggressive Cognition
*Objectives:*
...
### 4.1.2 Check sampling adequacy for Factor Analysis
...
# 5. Test Quality Criteria: Anxious Cognition
*Objectives:*
...
```{r}
cor_anx <- psych::tetrachoric(WFCT_anx_clean)$rho
...
## 5.2 Reliability
```{r}
psych::alpha(WFCT_anx_clean, check.keys = FALSE)
```
...
```{r}
summary_mean_sd(df, c("aggressiveWordCompletionScore", "anxiousWordCompletionScore"))
```
