...
tetra_corr <- tetrachoric(aggr_clean)$rho

# 3. KMO & Bartlett
KMO(tetra_corr)
cortest.bartlett(tetra_corr, n = nrow(df_threat))

# 4. EFA
fa(tetra_corr, nfactors = 1, n.obs = nrow(df_threat))

tetra_corr <- tetrachoric(aggr_clean)$rho
fa(tetra_corr, nfactors = 1, n.obs = nrow(df_threat))
model <- "
  Aggression =~ WFCTagg2 + WFCTagg3 + WFCTagg4  + WFCTagg6 + WFCTagg7 + WFCTagg10
"
fit <- cfa(model, data = aggr_clean, estimator = "WLSMV",
           ordered = colnames(aggr_clean))
summary(fit, fit.measures = TRUE, standardized = TRUE)
```

```{r}
bad_items <- c("WFCTagg1", "WFCTagg3","WFCTagg4", "WFCTagg5")
...
Interpretation:
-  The KMO is extremely low, indicating that the items do not firm a common
factor
- 
